...
Если случайная величина $X$ распределена нормально $N(\mu, \sigma^2)$, то построение интервала зависит от того, известна ли нам дисперсия генеральной совокупности.

**А. Дисперсия $\sigma^2$ известна**

В этом случае используется квантиль стандартного нормального распределения $z_{\alpha/2}$.Доверительный интервал для математического ожидания $\mu$:

$$\left( \bar{x} - z_{1-\alpha/2} \frac{\sigma}{\sqrt{n}}; \bar{x} + z_{1-\alpha/2} \frac{\sigma}{\sqrt{n}} \right)$$

**Б. Дисперсия $\sigma^2$ неизвестна (малая выборка)**

Мы заменяем $\sigma$ на выборочное отклонение $s$ и используем распределение Стьюдента.

$$\left( \bar{x} - t_{\alpha/2, n-1} \frac{s}{\sqrt{n}}; \bar{x} + t_{\alpha/2, n-1} \frac{s}{\sqrt{n}} \right)$$

**Для ненормальных распределений**

Если распределение признака отличается от нормального, подход меняется в зависимости от объема данных.

**А. Большие выборки ($n \to \infty$)**

Благодаря Центральной предельной теореме, распределение выборочного среднего $\bar{x}$ стремится к нормальному, даже если исходные данные распределены как угодно (экспоненциально, Пуассона и т.д.).

При $n > 30$ (а лучше $n > 100$) можно смело использовать формулы для нормального распределения (Z-интервал), подставляя выборочную дисперсию.

**Б. Малые выборки и известные законы**

Если выборка мала и распределение явно ненормальное, используются квантили соответствующих распределений:

Для дисперсии $\sigma^2$: Используются квантили распределения Хи-квадрат ($\chi^2$). Интервал получается несимметричным:

$$\left( \frac{(n-1)s^2}{\chi^2_{\alpha/2, n-1}}; \frac{(n-1)s^2}{\chi^2_{1-\alpha/2, n-1}} \right)$$

Для вероятности успеха $p$ (Бернулли): 

Используется аппроксимация или точный интервал Клоппера-Пирсона (через бета-распределение). (хч)
