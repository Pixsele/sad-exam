...
где $  s_{\bar{X}_1 - \bar{X}_2} = \sqrt{\frac{s_{\text{pooled}}^2}{n_1} + \frac{s_{\text{pooled}}^2}{n_2}}  $ — стандартная ошибка разности средних, $  s_{\text{pooled}}^2  $ — взвешенная дисперсия (объединенная дисперсия из обеих выборок). Степени свободы: df = n₁ + n₂ - 2.
