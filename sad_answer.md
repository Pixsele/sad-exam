...
- Границами этих областей служат критические точки (квантили распределения), которые находятся по таблицам (например, $z_{\alpha}$, $t_{crit}$, $F_{crit}$)
  ...
  - Рассчитывается наблюдаемое значение критерия $\Theta_{\text{набл}}$ (или $Z_{obs}, t_{obs}$) по выборке
  - Если наблюдаемое значение попадает в критическую область ($\Theta_{\text{набл}} \in S$), гипотеза $H_{0}$ отклоняется
  - Если значение попадает в область принятия ($\Theta_{\text{набл}} \in \overline{S}$), гипотеза $H_{0}$ принимается (не отклоняется)
...
$z_{\text{набл}} = 4.47$, $p\text{-value} \approx 2.8 \times 10^{-6}$, $H_0$ отвергается
...
Расчёт критических значений $z_{\text{кр}} = \pm 1.96$ при $\alpha = 0.05$. Если $z_{\text{набл}}$ попадает в критическую область, $H_0$ отвергается.
...
**Уровень значимости $\alpha$ ** — это вероятность отвергнуть нулевую гипотезу, когда она верна (вероятность ошибки первого рода).
**
...
При размере выборки >20 можно использовать нормальное распределение. $W \approx N\left(0, \frac{n(n+1)(2n+1)}{6}\right)$.
...
Определить значение $U$-статистики Манна-Уитни по формуле $U = \min\{U_1, U_2\}$.
...
D(внутр) = 12/5 = 2,4
D(межгр) = 8/9 ≈ 0,889
Проверка: 2,4 + 0,889 = 3,289 (верно)
...
Это уже не прямая, а **плоскость** (для трёх переменных) или **гиперплоскость** (для большего числа переменных).

**Условия Гаусса‑Маркова (свойства МНК‑оценок)**

$$y_i = a + b x_i + e_i, \quad i = 1, \ldots, n$$

1. $M[e_i] = 0$, $i = 1, \ldots, n$ — условие, гарантирующее несмещённость оценок МНК.

2. $D[e_i] = \sigma^2$, $i = 1, \ldots, n$ — условие гомоскедастичности.

3. $\text{cov}[e_i, e_j] = 0$ для всех $i \neq j$ — условие отсутствия автокорреляции предполагает отсутствие систематической связи между значениями случайного члена в любых двух наблюдениях.

4. $\text{cov}[e_i, x_j] = 0$ для всех $i \neq j$ — условие независимости случайного возмущения и объясняющей переменной. Значение любой независимой переменной в каждом наблюдении должно считаться экзогенным, полностью определяемым внешними причинами, не учитываемыми в уравнении регрессии.

**Теорема Гаусса‑Маркова:** если выполнены условия Гаусса‑Маркова, тогда оценки $\hat{a}, \hat{b}$, полученные с помощью метода наименьших квадратов, являются **линейными, несмещёнными, эффективными и состоятельными** оценками.

1. **Линейность оценок** — оценки параметров $a, b$ представляют собой линейные комбинации наблюдаемых значений объясняемой переменной $y_i$.
...
4. **Состоятельность оценок:**
   ...
   $$\operatorname{plim}_{n \to \infty} \hat{a} = a; \quad \operatorname{plim}_{n \to \infty} \hat{b} = b.$$  
   Чем больше наблюдений, тем ближе подсчитанные $a$ и $b$ к реальным.

5. **Эффективность** — означает, что оценка имеет минимальную дисперсию в заданном классе оценок:
...
(скорректированный коэффициент детерминации) лучше, чем просто $R^2$, так как не увеличивается с ростом количества переменных в модели  
...
Проверка корректности предположений модели.

1. Асимметрия и эксцесс относятся к форме распределения. Асимметрия - это мера асимметрии распределения, а эксцесс - это мера его кривизны, в частности, насколько заостренной является кривая.

$$S = \frac{\hat{μ}^3}{\hat{σ}^3} =
\left(\frac{1}{N} \sum_{i=1}^{N}(y_i - \hat{y}_i)^3 \right)
/
\left(\frac{1}{N}
\sum_{i=1}^{N} (y_i - \hat{y}_i)^2 \right)^{3/2}
$$

$$ k = \frac{\hat{μ}^4}{\hat{σ}^4} =
\left( \frac{1}{N} \sum_{i=1}^{N} (y_i - \hat{y}_i)^4 \right)
/
\left( \frac{1}{N} \sum_{i=1}^{N} (y_i - \hat{y}_i)^2 \right)^{2}
$$

2. Omnibus Test

Общий тест использует асимметрию и эксцесс для проверки нулевой гипотезы о том, что распределение является нормальным. В данном случае мы рассматриваем распределение остатка. Если мы получим очень малое значение для, значит, остаточные значения обычно не равны нулю, и нам, возможно, следует более внимательно изучить нашу модель.

3. Durbin-Watson

Тест Дурбина-Уотсона используется для определения наличия автокорреляции (взаимосвязи между значениями, отделенными друг от друга заданным временным лагом) в остатках. Автокорреляция остатков возникает, когда остатки текущего периода коррелируют с остатками предыдущего периода

$$DW = \frac{\sum_{i=2}^{N} \left( (y_i - \hat{y}_i) - (y_{i-1} - \hat{y}_{i-1}) \right)^2}{\sum_{i=1}^{N} \left( y_i - \hat{y}_i \right)^2}$$

Значение d всегда лежит между 0 и 4. Если d близко к 2, это означает, что автокорреляции нет, и мы принимаем нулевую гипотезу.

4. Jarque-Bera Test

Тест Жарка-Бера - это еще один тест, который учитывает асимметрию (S) и эксцесс (K). Нулевая гипотеза заключается в том, что распределение нормальное, что как асимметрия, так и избыточный эксцесс равны нулю, или, что асимметрия равна нулю, а обычный эксцесс равен трем.

$$JB = \frac{N}{6} \left( S^2 + \frac{1}{4} \left( K - 3 \right)^2 \right)$$

5. Condition Number

Число условий измеряет чувствительность выходных данных функции к ее входным данным. Когда две переменные-предиктора сильно коррелируют, что называется мультиколлинеарностью

Если меньше 30, следовательно модель является слабо мультиколлинеарной.
