...
Пусть предложения А–Е выполняются, тогда оценка МНК является:  
а) несмещённой,  
б) эффективной в классе несмещённых оценок, линейных по $y$.

**Предположение А.**  
На вектор неизвестных параметров регрессии (1.1) не наложено ограничений. Это значит, что $\Theta = \mathbb{R}^m$, где $\Theta$ — множество априорных значений параметров $\alpha$.

**Предположение Б.**  
Вектор $\varepsilon = (\varepsilon_1, \varepsilon_2, \ldots, \varepsilon_n)'$ — случайный. Отсюда следует, что $y = (y_1, y_2, \ldots, y_n)'$ — также случайный вектор.

**Предположение В.**  
Математическое ожидание $\mathbb{E}(\varepsilon_t)$ равно нулю, т.е. $\mathbb{E}(\varepsilon_t) = 0$, $t = 1, 2, \ldots, n$; $\mathbb{E}(\varepsilon) = 0$.

**Предположение Г.**  
Для любых $t_1 \neq t_2$ $\mathbb{E}(\varepsilon_{t_1} \varepsilon_{t_2}) = 0$, $\mathbb{E}(\varepsilon_t^2) = \sigma^2$ для всех $t = 1, 2, \ldots, n$.  
Другими словами, $\text{cov}(\varepsilon) = \sigma^2 I_n$. Здесь $\sigma^2$ — дисперсия отклонений; $\text{cov}(\varepsilon)$ — матрица ковариаций отклонений размерности $n \times n$; $I_n$ — единичная матрица размерности $n \times n$, т.е.
...
**Предположение Д.**  
Матрица $X$: детерминированная, т.е. $x_{it}$ не являются случайными переменными.
